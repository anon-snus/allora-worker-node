...
CYAN="\033[0;36m"
RESET="\033[0m"  # Fixed RESET color code for proper reset
...
# Fetching the latest Go version dynamically
LATEST_GO_VERSION=$(curl -s https://go.dev/VERSION?m=text) # e.g., go1.19.5
GO_URL="https://go.dev/dl/${LATEST_GO_VERSION}.linux-amd64.tar.gz"

# Check if the URL is accessible
if curl --output /dev/null --silent --head --fail "$GO_URL"; then
    execute_with_prompt "wget $GO_URL"
    execute_with_prompt "sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf ${LATEST_GO_VERSION}.linux-amd64.tar.gz"
    execute_with_prompt "rm ${LATEST_GO_VERSION}.linux-amd64.tar.gz"
    ...
    echo -e "${BOLD}${DARK_YELLOW}Error: Unable to download Go from $GO_URL. Please check the URL or the version number.${RESET}"
    exit 1
fi
echo
